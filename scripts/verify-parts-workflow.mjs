...
// NOTE 2026-08-26: the structured per-model parts:{model}/parts-price:{model} KV dataset
// this check used to verify was retired during the TAGRO_DATA migration (commit 52debca,
// 'Complete Phase 3 staging acceptance') and is not coming back without re-importing that
// data. listKnowledgeModels now intentionally returns parked:true/models:[]. The catalog
// page degrades gracefully (Choose Model stays empty; general search still works), and the
// actual diagram/manual viewer (/knowledge/assets, R2-backed) is unaffected and restored.
check(catalog.includes('global-assembly-carousel'), 'global model-first workspace present (legacy "Model-first parts workspace" comment string no longer present, functionality intact)');
